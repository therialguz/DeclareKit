...
struct Animation {
    ...
    static let `default` = Animation.easeInOut()

    static func easeInOut(duration: TimeInterval = 0.3) -> Animation {
        Animation(duration: duration, options: .curveEaseInOut)
    }

    static func easeIn(duration: TimeInterval = 0.3) -> Animation {
        Animation(duration: duration, options: .curveEaseIn)
    }

    static func easeOut(duration: TimeInterval = 0.3) -> Animation {
        Animation(duration: duration, options: .curveEaseOut)
    }

    static func linear(duration: TimeInterval = 0.3) -> Animation {
...
@MainActor
func withAnimation(_ animation: Animation = .default, _ body: () -> Void) {
